...
/**
 * 生成Md5、安装一个Apk、将资源目录下文件写入本地、获取一个apk的包信息、获取imsi码、imei和
 * 
 * mac地址的md5值、获取包名、获取版本号、获取版本名、获取Sim卡状态、
 * 
 * 获取电话号码、获取MAC地址、弹出进度条、关闭进度条对话框、弹出对话
 * 
 * 框、改变目标目录的访问权限、将一个流转换成字符串、给文件加时间戳
 * 
 * 、获取文件时间戳、检查SD卡是否存在、返回屏幕宽(px)、返回屏幕高
 * 
 * (px)、获取IMEI（国际移动设备身份码的缩写，国际移动装备辨识码）、
 * 
 * 获取手机型号、 启动一个通知栏、将long值大小格式化为有单位的KB MB
 * 
 * GB、获取手机自带可用内存rom.、获取手机总内存rom.、获取总运存大小
 * 
 * RAM、获取可用运存大小RAM、Sdcard的可用空间、Sdcard的总空间
 * 
 */
public class AppInfoUtil {
	public static final String SDCARD_SOFT_STORE = Environment
			.getExternalStorageDirectory() + "/htjxsdk/";
	...
	/**
	 * 方法描述：获取IMEI IMEI(International Mobile Equipment Identity)是国际移动设备身份码的缩写，
	 * 国际移动装备辨识码，是由15位数字组成的"电子串号"，它与每台手机一一对应，
	 * 而且该码是全世界唯一的。每一部手机在组装完成后都将被赋予一个全球唯一的一组号码，
	...
	public static void stratNotification(Context context, String contentTitle,
			String contentText, int icon, String pkName, String className) {
		// 1.获取通知管理器
		...
		if (pkName == null) {
			intent.setClassName(AppInfoUtil.getPkName(context),
					AppInfoUtil.getPkName(context) + ".MainActivity.class");
		} else {
